<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Laporan Transaksi Pembelian">
        <VCardText>
          <VRow>
            <VCol
              cols="3"
  ...
  methods: {
    async filterData() {
      if (this.bulan && this.tahun) {
        try {
          const response = await axios.get(`/api/transaksi-pembelian/filter?bulan=${this.bulan}&tahun=${this.tahun}`)
    ...
    async exportPDF() {
      if (this.bulan && this.tahun) {
        try {
          window.open(`/api/transaksi-pembelian/export-pdf?bulan=${this.bulan}&tahun=${this.tahun}`, '_blank')
        } catch (error) {
